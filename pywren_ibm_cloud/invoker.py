    ...
    def run(self, job_description):
        ...
        if job.remote_invocation:
            log_msg = ('ExecutorID {} | JobID {} - Starting {} remote invocation function: Spawning {}() '
                       '- Total: {} activations'.format(self.executor_id, job.job_id, job.total_tasks,
                                                        job.func_name, job.original_total_tasks))
        else:
            log_msg = ('ExecutorID {} | JobID {} - Starting function invocation: {}()  - Total: {} '
                       'activations'.format(self.executor_id, job.job_id, job.func_name, job.total_tasks))
        logger.info(log_msg)
        ...
        def invoke(executor_id, job_id, task_id, func_key, invoke_metadata, data_key, data_byte_range):

            output_key = create_output_key(self.storage_config['prefix'], executor_id, job_id, task_id)
            status_key = create_status_key(self.storage_config['prefix'], executor_id, job_id, task_id)
            ...
            if not activation_id:
                raise Exception("ExecutorID {} - Activation {} failed, therefore job is failed".format(executor_id, task_id))
            ...
            fut = ResponseFuture(task_id, job_id, executor_id, activation_id, self.storage_config, invoke_metadata)
        ...
        with ThreadPoolExecutor(max_workers=job.invoke_pool_threads) as executor:
            for i in range(job.total_tasks):
                task_id = "{:05d}".format(i)
                data_byte_range = job.data_ranges[i]
                future = executor.submit(invoke, self.executor_id,
                                         job.job_id, task_id, job.func_key,
