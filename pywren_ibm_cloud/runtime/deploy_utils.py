...
def create_runtime(name, memory=None, config=None):
    ...
    runtime_meta = compute.generate_runtime_meta(name)
    compute.create_runtime(name, memory, timeout=timeout)

    try:
        runtime_key = compute.get_runtime_key(name, memory)
...
def build_runtime(name, file, config=None):
    ...
    compute_config = extract_compute_config(config)
    compute = Compute(compute_config)
    compute.build_runtime(name, file)
...
def delete_runtime(name, config=None):
    ...
    runtimes = compute.list_runtimes(name)
    for runtime in runtimes:
        compute.delete_runtime(runtime[0], runtime[1])
        runtime_key = compute.get_runtime_key(runtime[0], runtime[1])
