...
class StandaloneHandler:
    """
    A standalone object is used by invokers and other components to access
    ...
    def __init__(self, standalone_config):
        ...
        try:
            module_location = 'lithops.standalone.backends.{}'.format(self.backend)
            sb_module = importlib.import_module(module_location)
            StandaloneBackend = getattr(sb_module, 'StandaloneBackend')
            self.standalone_backend = StandaloneBackend(self.config[self.backend])

        except Exception as e:
            logger.error("There was en error trying to create the {} standalone backend".format(self.backend))
            raise e
