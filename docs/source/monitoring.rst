...
monitoring approach polls the Object Store every few seconds to determine which function activations have finished
and which have not.

Since this default approach can slow down total application execution time due to the number of requests it has to
make against the object store, Lithops also integrates a RabbitMQ service to monitor function activations in real time.
