...
To customize the ``chunksize`` parameter, edit your ``map()`` or ``map_reduce()`` calls and specify the desired value, for example:
...
determined by the number of worker processes in the VM setup. Adjusting the number of VM instances or the configuration of
...
value for ``worker_processes``. Depending on the use case, it may be convenient to set more ``worker_processes`` than CPUs,
or fewer ``worker_processes`` than CPUs. For example, we can use a ``t2.medium`` instance type, which has 2 CPUs, but
