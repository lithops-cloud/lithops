...
* `local_executor()`: Localhost executor for running jobs locally.
* `serverless_executor()`: Serverless executor.
* `standalone_executor()`: Standalone executor or running jobs in a VM.
* `function_executor()`: Generic executor based on the executor set in configuration.
...
|runtime |  None | Name of the docker image to run the functions. If not provided, it will run the functions in the default python3 interpreter |
...
```python
import lithops
lex = lithops.local_executor()
```


**serverless_executor(\*\*kwargs)**
...
```python
import lithops
lex = lithops.serverless_executor()
```

**standalone_executor(\*\*kwargs)**

Initialize and return a Standalone executor object.
...
```python
import lithops
lex = lithops.standalone_executor()
```
