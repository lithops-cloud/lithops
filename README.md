...
### Multiple parameters
PyWren for IBM Cloud allows to send multiple parameters in the function invocation. See detailed examples on how to use multiple parameters [here](docs/multiple-parameters.md).

Multiple parameters in function allowed us to add some new built-in capabilities in PyWren, Thus, take into account that there are some parameter names that activate internal logic. These reserved parameters are:

- **id**: To get the call id. For example, if you spawn 10 activations of a function, you will get here a number from 0 to 9, for example: [map.py](examples/map.py)

- **ibm_cos**: To get a ready-to use [ibm_boto3.Client()](https://ibm.github.io/ibm-cos-sdk-python/reference/services/s3.html#client) instance. This allows you to access your IBM COS account from any function, for example: [ibmcos_arg.py](examples/ibmcos_arg.py)
...
It is possible to use **IBM-PyWren** inside **IBM Watson Studio** or Jupyter notebooks in order to run your workloads. You must ensure that the **IBM-PyWren** package is installed in the environment you are using the notebook. To do so, we recommend to add these lines at the beginning of the notebook:
